...
class VKStartSpam(
    StatesGroup): account_id = State(); template_id = State(); recipients_type = State(); interval = State()
...
async def check_vk_subscription(user_id: int) -> bool:
    with models.SessionLocal() as db:
        ...
        if not user or not user.vk_spammer_subscription_expires:
            return False
        if user.vk_spammer_subscription_expires < datetime.utcnow():
            user.vk_spammer_subscription_expires = None
            db.commit()
            return False
        return True
...
async def start_cmd(message: types.Message, bot: Bot):
    ...
    # Сброс состояния поддержки, если висит
    ...
    with models.SessionLocal() as db:
        ...
        if not user:
            user = models.User(tg_id=user_id, username=message.from_user.username,
                               full_name=message.from_user.full_name, balance=0.0)
            db.add(user);
            db.commit()
...
async def deposit_amount(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        user = db.query(models.User).filter_by(tg_id=user_id).first()
        if not user:
            user = models.User(tg_id=user_id);
            db.add(user);
            db.commit()
    try:
...
async def activate_promo_code(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if promo.expires_at and promo.expires_at < datetime.utcnow():
            await message.answer("⏰ Срок истёк");
            await state.clear();
            return
        ...
        if existing:
            await message.answer("❌ Вы уже активировали");
            await state.clear();
            return
        ...
        await message.answer(
            f"✅ Активирован! +{promo.amount} {CRYPTO_CURRENCY}. Баланс: {user.balance:.2f} {CRYPTO_CURRENCY}")
    await state.clear()
...
async def show_catalog(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not products:
            ...
            return
        await callback.message.edit_text("📂 <b>Каталог</b> (страница 1)", parse_mode="HTML",
                                         reply_markup=kb.catalog_keyboard(products, page=0))
    await callback.answer()
...
async def catalog_page(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not products:
            ...
            return
        await callback.message.edit_text(f"📂 <b>Каталог</b> (страница {page + 1})", parse_mode="HTML",
                                         reply_markup=kb.catalog_keyboard(products, page=page))
    await callback.answer()
...
async def view_product(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        await callback.message.edit_text(f"🌟 <b>{product.name}</b>\n\nВыберите аккаунт (страница 1):",
                                         parse_mode="HTML",
                                         reply_markup=kb.sessions_keyboard(sessions, product_id, page=0))
    await callback.answer()
...
async def sessions_page(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        await callback.message.edit_text(f"🌟 <b>{product.name}</b>\n\nВыберите аккаунт (страница {page + 1}):",
                                         parse_mode="HTML",
                                         reply_markup=kb.sessions_keyboard(sessions, product_id, page=page))
    await callback.answer()
...
async def buy_session(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        await callback.message.edit_text(text, parse_mode="HTML",
                                         reply_markup=kb.product_detail_keyboard(product.id, has_balance))
...
async def buy_with_balance(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        purchase = models.Purchase(user_id=user.id, product_id=product.id, session_id=session.id,
        ...
        await bot.send_message(callback.message.chat.id, "🔄 Обновить каталог?",
                               reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                   [InlineKeyboardButton("🛍 Обновить каталог", callback_data="catalog")]]))
    await callback.answer()
...
async def check_payment(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not invoice or invoice.status == "paid":
            await callback.message.edit_text("✅ Счёт уже оплачен");
            return
        ...
        if invoice.is_deposit:
            ...
            await callback.message.edit_text(
                f"✅ Баланс пополнен на {invoice.amount} {invoice.currency}\nНовый баланс: {user.balance:.2f} {invoice.currency}")
        else:
            ...
            if not session:
                ...
                return
            session.is_sold = True
            purchase = models.Purchase(user_id=user.id, product_id=invoice.product_id, session_id=session.id,
            ...
            if purchases_count == 1 and user.referred_by:
                ...
                if referrer:
                    ...
                    try:
                        await bot.send_message(referrer.tg_id,
                                               f"🎉 Реферал @{user.username} купил! Бонус {bonus} {invoice.currency}")
                    except:
                        pass
            await bot.send_message(callback.message.chat.id, "🔄 Обновить каталог?",
                                   reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def my_purchases(callback: types.CallbackQuery, bot: Bot):
    with models.SessionLocal() as db:
        ...
        purchases = db.query(models.Purchase).filter_by(user_id=user.id).order_by(
...
async def balance_history(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not user:
            ...
            return
        transactions = db.query(models.Transaction).filter_by(user_id=user.id).order_by(
            models.Transaction.created_at.desc()).limit(20).all()
        if not transactions:
...
# ------------------ Вывод бонусного баланса (с заявками) ------------------
...
async def withdraw_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    with models.SessionLocal() as db:
        ...
        if user.referral_balance <= 0:
            ...
            return
        await callback.message.edit_text(
            f"💸 <b>Вывод бонусного баланса</b>\n\n💰 Доступно: <code>{user.referral_balance:.2f} {CRYPTO_CURRENCY}</code>\n\nВведите сумму вывода (минимум 1):",
            parse_mode="HTML")
    ...
    with models.SessionLocal() as db:
        ...
        for admin_id in ADMIN_IDS:
            await bot.send_message(admin_id,
                                   ...
                                   parse_mode="HTML")
        await message.answer(
            f"✅ Заявка на вывод {amount} {CRYPTO_CURRENCY} создана. Ожидайте подтверждения администратора.")
    await state.clear()
...
async def admin_withdrawals(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        requests = db.query(models.WithdrawalRequest).filter_by(status="pending").order_by(
            models.WithdrawalRequest.created_at.desc()).all()
        if not requests:
        ...
        for r in requests:
            ...
            text += f"ID {r.id} | @{username} | {r.amount} {CRYPTO_CURRENCY} | {r.wallet}\n"
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Одобрить", callback_data=f"approve_withdraw_{r.id}") for r in requests[:3]],
...
async def approve_withdraw(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if user.referral_balance < req.amount:
            req.status = "rejected"
            db.commit()
        ...
        db.commit()
        add_transaction(db, user.id, -req.amount, "withdrawal", description=f"Вывод на кошелёк {req.wallet}")
        await log_action(bot, callback.from_user.id, "approve_withdraw", f"Заявка {req_id}, пользователь {user.tg_id}")
        await callback.answer("Заявка одобрена", show_alert=True)
        try:
            await bot.send_message(user.tg_id,
                                   f"✅ Ваша заявка на вывод {req.amount} {CRYPTO_CURRENCY} одобрена. Средства будут отправлены в ближайшее время.")
        except:
            pass
    await callback.message.delete()
    # Обновляем список заявок
    await admin_withdrawals(callback, bot)
...
async def admin_stats(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        total_deposits = db.query(models.Invoice).filter_by(is_deposit=True, status="paid").with_entities(
            func.sum(models.Invoice.amount)).scalar() or 0
...
async def add_product_currency(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        await message.answer(f"✅ Товар «{product.name}» добавлен. Цена: {product.price} {product.currency}",
...
async def change_price_new_price(message: types.Message, state: FSMContext, bot: Bot):
    try:
        ...
        with models.SessionLocal() as db:
            ...
            db.commit()
            await log_action(bot, message.from_user.id, "admin_change_price",
                             f"Товар ID {product_id}: цена изменена с {old_price} на {new_price} {product.currency}")
            await message.answer(
...
async def toggle_product_active(message: types.Message, state: FSMContext, bot: Bot):
    try:
        ...
        with models.SessionLocal() as db:
            ...
            await log_action(bot, message.from_user.id, "admin_toggle_product",
                             f"Товар ID {product_id} теперь {new_status}")
...
async def add_tdata_file(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        db.add(session);
        db.commit()
        await log_action(bot, message.from_user.id, "admin_add_tdata",
                         f"Добавлен tdata к товару ID {data['product_id']}")
        await message.answer(f"✅ Сессия добавлена. Файл: {file.file_name}, контактов: {data['contacts_count']}",
                             reply_markup=kb.admin_menu_keyboard())
    await state.clear()
...
async def add_text_data(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        db.add(session);
        db.commit()
        await log_action(bot, message.from_user.id, "admin_add_text",
                         f"Добавлена текстовая сессия к товару ID {data['product_id']}")
        await message.answer(f"✅ Текстовая сессия добавлена. Контактов: {data['contacts_count']}",
                             reply_markup=kb.admin_menu_keyboard())
    await state.clear()
    ...
    try:
        ...
        with models.SessionLocal() as db:
            ...
            if action == "add":
                ...
                await log_action(bot, message.from_user.id, "admin_add_balance",
                                 f"+{amount} {CRYPTO_CURRENCY} пользователю {user_id}")
                await message.answer(
                    f"✅ Добавлено {amount} {CRYPTO_CURRENCY}. Баланс: {user.balance:.2f} {CRYPTO_CURRENCY}")
            else:
                ...
                await log_action(bot, message.from_user.id, "admin_remove_balance",
                                 f"-{amount} {CRYPTO_CURRENCY} у пользователя {user_id}")
                await message.answer(
                    f"✅ Списано {amount} {CRYPTO_CURRENCY}. Баланс: {user.balance:.2f} {CRYPTO_CURRENCY}")
        await state.clear()
    except:
...
async def admin_ban_user_id(message: types.Message, state: FSMContext, bot: Bot):
    try:
        ...
        with models.SessionLocal() as db:
            ...
            await message.answer(
                f"👤 Пользователь {target_id} — {current}. Что сделать?\n/ban - забанить\n/unban - разбанить")
...
async def admin_export_data(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        data = {
            ...
            "sessions": [
                {"product_id": s.product_id, "data": s.data, "file_data": s.file_data.hex() if s.file_data else None,
            ...
            "purchases": [{"user_id": pu.user_id, "product_id": pu.product_id, "session_id": pu.session_id,
                           "paid_with_balance": pu.paid_with_balance, "purchased_at": pu.purchased_at.isoformat()} for
                          pu in purchases],
            ...
            "promo_activations": [
                {"user_id": a.user_id, "promo_id": a.promo_id, "activated_at": a.activated_at.isoformat()} for a in
                promo_acts],
            "logs": [
                {"user_id": l.user_id, "action": l.action, "details": l.details, "created_at": l.created_at.isoformat()}
                for l in logs],
        }
        ...
        db.add(backup);
        db.commit()
        await bot.send_document(callback.message.chat.id, BufferedInputFile(json_str.encode('utf-8'),
...
async def admin_import_data_file(message: types.Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await message.answer("❌ Ошибка парсинга JSON");
        return
    with models.SessionLocal() as db:
        ...
        for prod_data in data.get("products", []):
            prod = models.Product(**{k: v for k, v in prod_data.items() if
                                     k in ['name', 'description', 'price', 'currency', 'is_active']})
            db.add(prod)
        db.flush()
...
async def vk_spammer_menu(callback: types.CallbackQuery, bot: Bot):
    if not await check_vk_subscription(callback.from_user.id):
        await callback.message.edit_text(
            f"❌ У вас нет активной подписки на VK Спаммер.\n\n💰 Стоимость подписки: {SPAMMER_SUBSCRIPTION_PRICE} {CRYPTO_CURRENCY}\n👉 Нажмите /buy_spammer, чтобы оплатить.",
            parse_mode="HTML")
        await callback.answer()
        return
    ...
    with models.SessionLocal() as db:
        ...
        if user.balance >= SPAMMER_SUBSCRIPTION_PRICE:
            ...
            try:
                ...
                db.commit()
                await message.answer(
                    f"💸 Счёт на {SPAMMER_SUBSCRIPTION_PRICE} {CRYPTO_CURRENCY} создан.\nОплатите и нажмите «Проверить оплату».",
...
async def vk_add_account_token(message: types.Message, state: FSMContext, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        account = models.VKAccount(
            ...
            subscription_expires=user.vk_spammer_subscription_expires  # синхронизируем
        )
        db.add(account);
        db.commit()
...
async def vk_my_accounts(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not accounts:
            await callback.message.edit_text("У вас нет добавленных аккаунтов VK.",
                                             reply_markup=kb.vk_spammer_menu_keyboard())
            return
        await callback.message.edit_text("📊 <b>Ваши аккаунты VK</b>", parse_mode="HTML",
                                         reply_markup=kb.vk_accounts_keyboard(accounts))
    await callback.answer()
...
async def vk_templates_menu(callback: types.CallbackQuery, bot: Bot):
    ...
    with models.SessionLocal() as db:
        ...
        if not templates:
            await callback.message.edit_text("У вас нет шаблонов. Создайте первый.",
                                             reply_markup=kb.vk_templates_keyboard([]))
            return
        await callback.message.edit_text(
            "📝 <b>Ваши шаблоны сообщений</b>\n\nВыберите шаблон для использования или создайте новый:",
            parse_mode="HTML", reply_markup=kb.vk_templates_keyboard(templates))
    await callback.answer()
...
async def vk_use_template(callback: types.CallbackQuery, state: FSMContext):
    ...
    with models.SessionLocal() as db:
        ...
        await callback.message.edit_text("📨 <b>Запуск рассылки</b>\n\nВыберите аккаунт VK:", parse_mode="HTML",
                                         reply_markup=kb.vk_accounts_keyboard(
                                             db.query(models.VKAccount).filter_by(user_id=user.id).all()))
